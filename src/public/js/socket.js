...
const tweets = document.getElementsByClassName('app--tweet--list')[0];

socket.on('twitter-stream', function (data) {
  //If on a tweets list is more than 5 tweets, remove last tweet
  if ($('.app--tweet--list>li').length >= 5) {
    $('.app--tweet--list>li').last().remove();
  }
  //Tweet template
  let newTweet = `<li><strong class="app--tweet--timestamp">${data.date}</strong>
  <a class="app--tweet--author">
    <div class="app--avatar" style="background-image: url(${data.imageURL})">
      <img src=${data.imageURL} />
    </div>
    <h4>${data.name}</h4> @${data.screen_name}
  </a>
  <p>${data.text}</p>
  ...
  //Adding new tweet to tweets list as a first item
  tweets.innerHTML = newTweet + tweets.innerHTML;
});
